...
class QLearningAgent:
    ...
    def get_best_policy(self, state):
        """Compute the best action to take in
        the state using Policy Extraction.
        π(s) = argmax_a Q(s,a)
    ...
    def update(self, state, action, next_state, reward):
        ...
        q_prime = ((1 - self.alpha) * q_value +
                   self.alpha * (reward + self.gamma * next_value))

        # update
        self.q_values[(state, action)] = q_prime
...
class ApproximateQAgent(QLearningAgent):
    ...
    def get_q_value(self, state, action):
        ...
        for feature in features:
            q_prime += self.get_weight(feature) * features[feature]
        return q_prime
    ...
    def update(self, state, action, next_state, reward):
        ...
        delta = (reward +
                 self.gamma * self.get_value(next_state) -
                 self.get_q_value(state, action))

        for feature in features:
            new_weight = self.alpha * delta * features[feature]
...
feedback_question_2 = """
I'm still not fully understanding the updates
even as I follow through alongside the lecture
slides.
"""

feedback_question_3 = """
this got to pacman, felt some nostalgia
"""
